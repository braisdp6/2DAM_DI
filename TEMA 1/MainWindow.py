# Form implementation generated from reading ui file '.\templates\MainWindow.ui'
#
# Created by: PyQt6 UI code generator 6.5.3
#
# WARNING: Any manual changes made to this file will be lost when pyuic6 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt6 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(1113, 768)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Expanding)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(MainWindow.sizePolicy().hasHeightForWidth())
        MainWindow.setSizePolicy(sizePolicy)
        MainWindow.setMinimumSize(QtCore.QSize(1024, 768))
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("C:/Users/a19braisdp/.designer/backup/img/taxi.ico"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        MainWindow.setWindowIcon(icon)
        MainWindow.setIconSize(QtCore.QSize(24, 24))
        self.centralwidget = QtWidgets.QWidget(parent=MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.gridLayout = QtWidgets.QGridLayout(self.centralwidget)
        self.gridLayout.setObjectName("gridLayout")
        self.panelPrincipal = QtWidgets.QTabWidget(parent=self.centralwidget)
        self.panelPrincipal.setMinimumSize(QtCore.QSize(980, 700))
        self.panelPrincipal.setObjectName("panelPrincipal")
        self.panelDrivers = QtWidgets.QWidget()
        self.panelDrivers.setObjectName("panelDrivers")
        self.frameConductores = QtWidgets.QFrame(parent=self.panelDrivers)
        self.frameConductores.setGeometry(QtCore.QRect(60, 30, 961, 251))
        self.frameConductores.setFrameShape(QtWidgets.QFrame.Shape.StyledPanel)
        self.frameConductores.setFrameShadow(QtWidgets.QFrame.Shadow.Plain)
        self.frameConductores.setLineWidth(5)
        self.frameConductores.setMidLineWidth(0)
        self.frameConductores.setObjectName("frameConductores")
        self.txtApel = QtWidgets.QLineEdit(parent=self.frameConductores)
        self.txtApel.setGeometry(QtCore.QRect(90, 60, 291, 21))
        self.txtApel.setMinimumSize(QtCore.QSize(0, 0))
        self.txtApel.setObjectName("txtApel")
        self.lblFechaAlta = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblFechaAlta.setGeometry(QtCore.QRect(520, 20, 71, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblFechaAlta.setFont(font)
        self.lblFechaAlta.setStyleSheet("")
        self.lblFechaAlta.setObjectName("lblFechaAlta")
        self.txtDni = QtWidgets.QLineEdit(parent=self.frameConductores)
        self.txtDni.setGeometry(QtCore.QRect(230, 20, 151, 21))
        self.txtDni.setMinimumSize(QtCore.QSize(120, 20))
        self.txtDni.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
        self.txtDni.setObjectName("txtDni")
        self.txtNombre = QtWidgets.QLineEdit(parent=self.frameConductores)
        self.txtNombre.setGeometry(QtCore.QRect(460, 60, 301, 21))
        self.txtNombre.setMinimumSize(QtCore.QSize(120, 20))
        self.txtNombre.setObjectName("txtNombre")
        self.lblNombre = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblNombre.setGeometry(QtCore.QRect(400, 60, 50, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblNombre.setFont(font)
        self.lblNombre.setStyleSheet("")
        self.lblNombre.setObjectName("lblNombre")
        self.lblApel = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblApel.setGeometry(QtCore.QRect(20, 60, 90, 20))
        self.lblApel.setMinimumSize(QtCore.QSize(60, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblApel.setFont(font)
        self.lblApel.setObjectName("lblApel")
        self.lblDni = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblDni.setGeometry(QtCore.QRect(200, 20, 50, 20))
        self.lblDni.setMinimumSize(QtCore.QSize(0, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblDni.setFont(font)
        self.lblDni.setObjectName("lblDni")
        self.lblCodbd = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblCodbd.setGeometry(QtCore.QRect(70, 20, 101, 21))
        self.lblCodbd.setMinimumSize(QtCore.QSize(100, 0))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblCodbd.setFont(font)
        self.lblCodbd.setStyleSheet("background-color: rgb(255, 253, 167);")
        self.lblCodbd.setText("")
        self.lblCodbd.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
        self.lblCodbd.setObjectName("lblCodbd")
        self.txtFechaAlta = QtWidgets.QLineEdit(parent=self.frameConductores)
        self.txtFechaAlta.setGeometry(QtCore.QRect(600, 20, 131, 21))
        self.txtFechaAlta.setMinimumSize(QtCore.QSize(120, 20))
        self.txtFechaAlta.setObjectName("txtFechaAlta")
        self.lblCod = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblCod.setGeometry(QtCore.QRect(20, 20, 50, 20))
        self.lblCod.setMinimumSize(QtCore.QSize(0, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblCod.setFont(font)
        self.lblCod.setObjectName("lblCod")
        self.btnCalendar = QtWidgets.QPushButton(parent=self.frameConductores)
        self.btnCalendar.setGeometry(QtCore.QRect(740, 20, 21, 21))
        self.btnCalendar.setText("")
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap("C:/Users/a19braisdp/.designer/backup/img/calendar.ico"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        self.btnCalendar.setIcon(icon1)
        self.btnCalendar.setObjectName("btnCalendar")
        self.lblValidarDni = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblValidarDni.setGeometry(QtCore.QRect(390, 10, 50, 41))
        self.lblValidarDni.setMinimumSize(QtCore.QSize(50, 30))
        self.lblValidarDni.setMaximumSize(QtCore.QSize(100, 100))
        font = QtGui.QFont()
        font.setFamily("Ravie")
        font.setPointSize(10)
        self.lblValidarDni.setFont(font)
        self.lblValidarDni.setText("")
        self.lblValidarDni.setAlignment(QtCore.Qt.AlignmentFlag.AlignLeading|QtCore.Qt.AlignmentFlag.AlignLeft|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.lblValidarDni.setObjectName("lblValidarDni")
        self.lblDireccion = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblDireccion.setGeometry(QtCore.QRect(20, 100, 90, 20))
        self.lblDireccion.setMinimumSize(QtCore.QSize(60, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblDireccion.setFont(font)
        self.lblDireccion.setObjectName("lblDireccion")
        self.txtDireccion = QtWidgets.QLineEdit(parent=self.frameConductores)
        self.txtDireccion.setGeometry(QtCore.QRect(90, 100, 371, 21))
        self.txtDireccion.setMinimumSize(QtCore.QSize(0, 0))
        self.txtDireccion.setObjectName("txtDireccion")
        self.lblMovil = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblMovil.setGeometry(QtCore.QRect(40, 140, 60, 20))
        self.lblMovil.setMinimumSize(QtCore.QSize(60, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblMovil.setFont(font)
        self.lblMovil.setStyleSheet("")
        self.lblMovil.setObjectName("lblMovil")
        self.txtMovil = QtWidgets.QLineEdit(parent=self.frameConductores)
        self.txtMovil.setGeometry(QtCore.QRect(90, 140, 181, 21))
        self.txtMovil.setMinimumSize(QtCore.QSize(120, 20))
        self.txtMovil.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
        self.txtMovil.setObjectName("txtMovil")
        self.lblProvincia = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblProvincia.setGeometry(QtCore.QRect(480, 100, 61, 20))
        self.lblProvincia.setMinimumSize(QtCore.QSize(50, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblProvincia.setFont(font)
        self.lblProvincia.setObjectName("lblProvincia")
        self.lblLocalidad = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblLocalidad.setGeometry(QtCore.QRect(710, 100, 81, 20))
        self.lblLocalidad.setMinimumSize(QtCore.QSize(50, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblLocalidad.setFont(font)
        self.lblLocalidad.setObjectName("lblLocalidad")
        self.cmbProvincia = QtWidgets.QComboBox(parent=self.frameConductores)
        self.cmbProvincia.setGeometry(QtCore.QRect(550, 100, 141, 22))
        self.cmbProvincia.setObjectName("cmbProvincia")
        self.cmbLocalidad = QtWidgets.QComboBox(parent=self.frameConductores)
        self.cmbLocalidad.setGeometry(QtCore.QRect(780, 100, 161, 22))
        self.cmbLocalidad.setObjectName("cmbLocalidad")
        self.lblSalario = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblSalario.setGeometry(QtCore.QRect(290, 140, 60, 20))
        self.lblSalario.setMinimumSize(QtCore.QSize(60, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblSalario.setFont(font)
        self.lblSalario.setStyleSheet("")
        self.lblSalario.setObjectName("lblSalario")
        self.txtSalario = QtWidgets.QLineEdit(parent=self.frameConductores)
        self.txtSalario.setGeometry(QtCore.QRect(340, 140, 120, 21))
        self.txtSalario.setMinimumSize(QtCore.QSize(120, 20))
        self.txtSalario.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.txtSalario.setObjectName("txtSalario")
        self.lblCarnet = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblCarnet.setGeometry(QtCore.QRect(10, 180, 91, 20))
        self.lblCarnet.setMinimumSize(QtCore.QSize(60, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblCarnet.setFont(font)
        self.lblCarnet.setStyleSheet("")
        self.lblCarnet.setObjectName("lblCarnet")
        self.chkA = QtWidgets.QCheckBox(parent=self.frameConductores)
        self.chkA.setGeometry(QtCore.QRect(110, 180, 31, 20))
        self.chkA.setMinimumSize(QtCore.QSize(0, 20))
        self.chkA.setObjectName("chkA")
        self.chkB = QtWidgets.QCheckBox(parent=self.frameConductores)
        self.chkB.setGeometry(QtCore.QRect(150, 180, 31, 20))
        self.chkB.setMinimumSize(QtCore.QSize(0, 20))
        self.chkB.setObjectName("chkB")
        self.chkC = QtWidgets.QCheckBox(parent=self.frameConductores)
        self.chkC.setGeometry(QtCore.QRect(190, 180, 31, 20))
        self.chkC.setMinimumSize(QtCore.QSize(0, 20))
        self.chkC.setObjectName("chkC")
        self.chkD = QtWidgets.QCheckBox(parent=self.frameConductores)
        self.chkD.setGeometry(QtCore.QRect(230, 180, 31, 20))
        self.chkD.setMinimumSize(QtCore.QSize(0, 20))
        self.chkD.setObjectName("chkD")
        self.lblEstado = QtWidgets.QLabel(parent=self.frameConductores)
        self.lblEstado.setGeometry(QtCore.QRect(490, 140, 41, 20))
        self.lblEstado.setMinimumSize(QtCore.QSize(0, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.lblEstado.setFont(font)
        self.lblEstado.setStyleSheet("")
        self.lblEstado.setObjectName("lblEstado")
        self.rbtTodos = QtWidgets.QRadioButton(parent=self.frameConductores)
        self.rbtTodos.setGeometry(QtCore.QRect(540, 140, 51, 20))
        self.rbtTodos.setMinimumSize(QtCore.QSize(0, 20))
        self.rbtTodos.setChecked(True)
        self.rbtTodos.setObjectName("rbtTodos")
        self.buttonGroup = QtWidgets.QButtonGroup(MainWindow)
        self.buttonGroup.setObjectName("buttonGroup")
        self.buttonGroup.addButton(self.rbtTodos)
        self.rbtBaja = QtWidgets.QRadioButton(parent=self.frameConductores)
        self.rbtBaja.setGeometry(QtCore.QRect(650, 140, 51, 20))
        self.rbtBaja.setMinimumSize(QtCore.QSize(0, 20))
        self.rbtBaja.setObjectName("rbtBaja")
        self.buttonGroup.addButton(self.rbtBaja)
        self.rbtAlta = QtWidgets.QRadioButton(parent=self.frameConductores)
        self.rbtAlta.setGeometry(QtCore.QRect(600, 140, 51, 20))
        self.rbtAlta.setMinimumSize(QtCore.QSize(0, 20))
        self.rbtAlta.setObjectName("rbtAlta")
        self.buttonGroup.addButton(self.rbtAlta)
        self.btnAltaDriver = QtWidgets.QPushButton(parent=self.frameConductores)
        self.btnAltaDriver.setGeometry(QtCore.QRect(340, 210, 75, 23))
        self.btnAltaDriver.setObjectName("btnAltaDriver")
        self.btnModificarDriver = QtWidgets.QPushButton(parent=self.frameConductores)
        self.btnModificarDriver.setGeometry(QtCore.QRect(440, 210, 75, 23))
        self.btnModificarDriver.setObjectName("btnModificarDriver")
        self.btnBajaDriver = QtWidgets.QPushButton(parent=self.frameConductores)
        self.btnBajaDriver.setGeometry(QtCore.QRect(540, 210, 75, 23))
        self.btnBajaDriver.setObjectName("btnBajaDriver")
        self.tabDrivers = QtWidgets.QTableWidget(parent=self.panelDrivers)
        self.tabDrivers.setGeometry(QtCore.QRect(60, 290, 961, 361))
        self.tabDrivers.setAlternatingRowColors(True)
        self.tabDrivers.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectionBehavior.SelectRows)
        self.tabDrivers.setObjectName("tabDrivers")
        self.tabDrivers.setColumnCount(6)
        self.tabDrivers.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.tabDrivers.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.tabDrivers.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.tabDrivers.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.tabDrivers.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.tabDrivers.setHorizontalHeaderItem(4, item)
        item = QtWidgets.QTableWidgetItem()
        self.tabDrivers.setHorizontalHeaderItem(5, item)
        self.tabDrivers.verticalHeader().setVisible(False)
        self.panelPrincipal.addTab(self.panelDrivers, "")
        self.tab_2 = QtWidgets.QWidget()
        self.tab_2.setObjectName("tab_2")
        self.panelPrincipal.addTab(self.tab_2, "")
        self.gridLayout.addWidget(self.panelPrincipal, 0, 0, 1, 1)
        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 1113, 21))
        self.menubar.setObjectName("menubar")
        self.menuArchivo = QtWidgets.QMenu(parent=self.menubar)
        self.menuArchivo.setObjectName("menuArchivo")
        self.menuHelp = QtWidgets.QMenu(parent=self.menubar)
        self.menuHelp.setObjectName("menuHelp")
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)
        self.toolBar = QtWidgets.QToolBar(parent=MainWindow)
        self.toolBar.setObjectName("toolBar")
        MainWindow.addToolBar(QtCore.Qt.ToolBarArea.TopToolBarArea, self.toolBar)
        self.actionSalir = QtGui.QAction(parent=MainWindow)
        self.actionSalir.setObjectName("actionSalir")
        self.actionAcerca_de = QtGui.QAction(parent=MainWindow)
        self.actionAcerca_de.setObjectName("actionAcerca_de")
        self.actionBarSalir = QtGui.QAction(parent=MainWindow)
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap("C:/Users/a19braisdp/.designer/backup/img/salir.ico"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        self.actionBarSalir.setIcon(icon2)
        self.actionBarSalir.setObjectName("actionBarSalir")
        self.actionLimpiarPanel = QtGui.QAction(parent=MainWindow)
        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap("C:/Users/a19braisdp/.designer/backup/img/limpiar.ico"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.On)
        self.actionLimpiarPanel.setIcon(icon3)
        self.actionLimpiarPanel.setObjectName("actionLimpiarPanel")
        self.menuArchivo.addAction(self.actionSalir)
        self.menuHelp.addAction(self.actionAcerca_de)
        self.menubar.addAction(self.menuArchivo.menuAction())
        self.menubar.addAction(self.menuHelp.menuAction())
        self.toolBar.addAction(self.actionBarSalir)
        self.toolBar.addSeparator()
        self.toolBar.addAction(self.actionLimpiarPanel)

        self.retranslateUi(MainWindow)
        self.panelPrincipal.setCurrentIndex(0)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)
        MainWindow.setTabOrder(self.panelPrincipal, self.txtDni)
        MainWindow.setTabOrder(self.txtDni, self.txtFechaAlta)
        MainWindow.setTabOrder(self.txtFechaAlta, self.btnCalendar)
        MainWindow.setTabOrder(self.btnCalendar, self.txtApel)
        MainWindow.setTabOrder(self.txtApel, self.txtNombre)
        MainWindow.setTabOrder(self.txtNombre, self.txtDireccion)
        MainWindow.setTabOrder(self.txtDireccion, self.cmbProvincia)
        MainWindow.setTabOrder(self.cmbProvincia, self.cmbLocalidad)
        MainWindow.setTabOrder(self.cmbLocalidad, self.txtMovil)
        MainWindow.setTabOrder(self.txtMovil, self.txtSalario)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.lblFechaAlta.setText(_translate("MainWindow", "Fecha Alta:"))
        self.lblNombre.setText(_translate("MainWindow", "Nombre:"))
        self.lblApel.setText(_translate("MainWindow", "Apellidos:"))
        self.lblDni.setText(_translate("MainWindow", "DNI:"))
        self.lblCod.setText(_translate("MainWindow", "Código:"))
        self.lblDireccion.setText(_translate("MainWindow", "Dirección:"))
        self.lblMovil.setText(_translate("MainWindow", "Móvil:"))
        self.lblProvincia.setText(_translate("MainWindow", "Provincia:"))
        self.lblLocalidad.setText(_translate("MainWindow", "Localidad:"))
        self.lblSalario.setText(_translate("MainWindow", "Salario:"))
        self.lblCarnet.setText(_translate("MainWindow", "Tipo de Carnet:"))
        self.chkA.setText(_translate("MainWindow", "A"))
        self.chkB.setText(_translate("MainWindow", "B"))
        self.chkC.setText(_translate("MainWindow", "C"))
        self.chkD.setText(_translate("MainWindow", "D"))
        self.lblEstado.setText(_translate("MainWindow", "Estado:"))
        self.rbtTodos.setText(_translate("MainWindow", "Todos"))
        self.rbtBaja.setText(_translate("MainWindow", "Baja"))
        self.rbtAlta.setText(_translate("MainWindow", "Alta"))
        self.btnAltaDriver.setText(_translate("MainWindow", "Alta"))
        self.btnModificarDriver.setText(_translate("MainWindow", "Modificar"))
        self.btnBajaDriver.setText(_translate("MainWindow", "Baja"))
        item = self.tabDrivers.horizontalHeaderItem(0)
        item.setText(_translate("MainWindow", "Código"))
        item = self.tabDrivers.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "Apellidos"))
        item = self.tabDrivers.horizontalHeaderItem(2)
        item.setText(_translate("MainWindow", "Nombre"))
        item = self.tabDrivers.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "Móvil"))
        item = self.tabDrivers.horizontalHeaderItem(4)
        item.setText(_translate("MainWindow", "Licencias"))
        item = self.tabDrivers.horizontalHeaderItem(5)
        item.setText(_translate("MainWindow", "Fecha Baja"))
        self.panelPrincipal.setTabText(self.panelPrincipal.indexOf(self.panelDrivers), _translate("MainWindow", "CONDUCTORES"))
        self.panelPrincipal.setTabText(self.panelPrincipal.indexOf(self.tab_2), _translate("MainWindow", "Tab 2"))
        self.menuArchivo.setTitle(_translate("MainWindow", "Archivo"))
        self.menuHelp.setTitle(_translate("MainWindow", "Ayuda"))
        self.toolBar.setWindowTitle(_translate("MainWindow", "toolBar"))
        self.actionSalir.setText(_translate("MainWindow", "Salir"))
        self.actionAcerca_de.setText(_translate("MainWindow", "Acerca de..."))
        self.actionBarSalir.setText(_translate("MainWindow", "Salir"))
        self.actionLimpiarPanel.setText(_translate("MainWindow", "limpiarPanel"))
